...
void ssd1306_draw_char(ssd1306_t *ssd, char c, uint8_t x, uint8_t y) {
    ...
    // Trata caracteres maiúsculos (A-Z)
    if (c >= 'A' && c <= 'Z') {
        index = (c - 'A' + 11) * 8;
    }
    // Trata dígitos (0-9)
    ...
    // Trata caracteres minúsculos (a-z)
    else if (c >= 'a' && c <= 'z') {
        index = (c - 'a' + 37) * 8; // Índice para minúsculas começa após Z
    }
    // Caractere inválido (ou espaço)
    else {
...
void ssd1306_draw_string(ssd1306_t *ssd, const char *str, uint8_t x, uint8_t y) {
    while (*str) {
        // Verifica se há espaço suficiente para desenhar o próximo caractere
        if (x + 8 >= ssd->width) {
            x = 0; // Volta ao início da próxima linha
            y += 8;
        }
        // Verifica se a altura máxima foi atingida
        if (y + 8 >= ssd->height) {
            break; // Não há mais espaço no display
        }

        // Desenha o caractere atual
        ssd1306_draw_char(ssd, *str++, x, y);
        x += 8; // Move para a próxima posição
    }
}
